import React, { useEffect } from "react";

// ** Reactstrap
import { Col, Row } from "reactstrap";

// ** 3rd Party Library
import { useParams } from "react-router-dom";

// ** Redux
import { useDispatch, useSelector } from "react-redux";
import { clearIntroducer, introducerProfileApi } from "../../redux/introducers/slice";

// SCSS
import '../../assets/scss/custom.scss'
import moment from "moment";
import { endUserListCount } from "../../redux/endusers/slice";
import playerdanger from '../../assets/images/logo/golfer-danger.png'
import playersuccess from '../../assets/images/logo/golfer-successs.png'

// ** Profile
const Profile = () => {
    const { id } = useParams();
    let { profile } = useSelector((store) => (store.introducer))
    const { endUserCount, reload } = useSelector((store) => (store.enduser))

    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(introducerProfileApi({ id }));
        return () => {
            dispatch(clearIntroducer())
        }
    }, [dispatch, id]);

    useEffect(() => {
        if (reload !== null) {
            dispatch(endUserListCount({ id }))
        }
    }, [reload])

    return (
        <>
            <div className="page-left-section bg-white shadow rounded p-2">
                <Row>
                    {/* Profile Info */}
                    <Col lg="12" md="5">
                        <div className="mb-1">
                            <div className="profile-img mx-auto pb-1 d-flex align-items-center border-bottom">
                                <img
                                    src={profile?.image}
                                    alt="profile"
                                    className="rounded-circle img-view"
                                    height='50'
                                    width='50'
                                />
                                <div className="ms-1">
                                    <p className="mb-0 profile-card-name">{`${profile?.firstName} ${profile?.lastName}`}</p>
                                    <p className="mb-0 profile-card-update">Joined at {moment(profile?.createdAt).format('D MMM YYYY')}</p>
                                </div>
                            </div>
                        </div>
                        <span>
                            <div className="pb-1 mb-1 rounded border-bottom">
                                <Row>
                                    <Col xl="6">
                                        <div className="d-flex align-items-center mb-xl-0 mb-1">
                                            <div className="avatar bg-lite-yahoo rounded m-0 introducer-card">
                                                <img src={playerdanger} width={25} height={25} className='rounded-0' />
                                            </div>
                                            <div className="ms-1">
                                                <h5 className="lh-1">{endUserCount?.TotalEndUser}</h5>
                                                <p className="mb-0 lh-1">Golf Players</p>
                                            </div>
                                        </div>
                                    </Col>
                                    <Col xl="6">
                                        <div className="d-flex align-items-center mb-xl-0 mb-1">
                                            <div className="avatar bg-lite-yahoo rounded m-0 introducer-card">
                                                <img src={playersuccess} width={25} height={25} className='rounded-0' />
                                            </div>
                                            <div className="ms-1">
                                                <h5 className="lh-1">{endUserCount?.TotalActiveEndUser}</h5>
                                                <p className="mb-0 lh-1">Active Players</p>
                                            </div>
                                        </div>
                                    </Col>
                                </Row>
                            </div>
                        </span>
                    </Col>
                    {/* Basic Details */}
                    <Col lg="12" md="7">
                        <div className="mb-xl-3 mb-1 profile-card">
                            <div>
                                <div className="d-flex align-items-center flex-wrap mb-1">
                                    <p className="fw-bold label-text mb-0">Email : </p>
                                    <p className="mb-0">{profile?.email}</p>
                                </div>
                                <div className="d-flex align-items-center flex-wrap mb-1">
                                    <p className="fw-bold label-text mb-0">Status :</p>
                                    {profile?.isActive ? <span>
                                        <div className="badge badge-light-success badge-pill mt-0 rounded">
                                            Active
                                        </div>
                                    </span> : <span>
                                        <div className="badge badge-light-danger badge-pill mt-0 rounded">
                                            Deactive
                                        </div>
                                    </span>}
                                </div>
                                <div className="d-flex align-items-center flex-wrap mb-1">
                                    <p className="fw-bold label-text mb-0">
                                        Address :
                                    </p>
                                    <p className="mb-0">
                                        {profile?.address}
                                    </p>
                                </div>
                                <div className="d-flex align-items-center flex-wrap mb-1">
                                    <p className="fw-bold label-text mb-0">
                                        Country :
                                    </p>
                                    <p className="mb-0">
                                        {profile?.country}
                                    </p>
                                </div>
                                <div className="d-flex align-items-center flex-wrap mb-1">
                                    <p className="fw-bold label-text mb-0">
                                        Contact :
                                    </p>
                                    <p className="mb-0">{profile?.mobileNo}</p>
                                </div>
                                <div className="d-flex align-items-center flex-wrap mb-1">
                                    <p className="fw-bold label-text mb-0">
                                        Introducer Type :
                                    </p>
                                    <p className="mb-0">{profile?.introducerType?.role}</p>
                                </div>
                            </div>
                        </div>
                    </Col>
                </Row >
                {/* Footer */}
                {/* <div
                        className="text-lg-center text-md-left"
                        hidden={!Permission.includes(roleId)}
                    >
                        <Button
                            className="mr-1"
                            color="info"
                        >
                            Edit
                        </Button>
                        <Button
                            onClick={() => deleteHandle(data?.id)}
                            color="outline-danger"
                        >
                            Suspend
                        </Button>
                    </div> */}
                {/* Edit Staff User */}
                {/* <EditStaffUser
            id={id}
          /> */}
            </div >
        </>
    );
};

export default Profile;
